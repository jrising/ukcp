...
for (rr in ukrrs){
  ...
  subdf <- df[df$region == nuts[rr] & df$impact == "Sea_level_rise",]
  if (nrow(subdf)==0){
    print(paste("Skipping", nuts[rr], "Sea_level_rise"))
    next 
  }
  print(paste("Processing", nuts[rr], "Sea_level_rise"))
...
gp <- ggplot(ddf, aes(T, -mu)) +
  ...
  theme_bw() + ylab("Damages to sea level rise (£ million)")
ggsave("../channels/SLR/SLR-coacch-df.pdf", width=9.5, height=9.5)

save(allla, file="../channels/SLR/SLR-la.RData")
