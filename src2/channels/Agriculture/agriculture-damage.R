...
ag.gcm.rcp <- read.csv("~/Open Modeling Group Dropbox/COACCH/climate/cmip5/Combine-GCM/agri-gcm-rcp.csv") 
ag.gcm.hist <-read.csv("~/Open Modeling Group Dropbox/COACCH/climate/cmip5/Combine-GCM/agri-gcm-hist.csv") 

timer <- ag.gcm.rcp$yyyymm
tsavg.r <-ag.gcm.rcp$tsavg
region.r <- ag.gcm.rcp$NUTS_ID
date.r <- as.Date(paste0(timer, "-15"))

timeh <- ag.gcm.hist$yyyymm
tsavg.h <-ag.gcm.hist$tsavg
region.h <- ag.gcm.hist$NUTS_ID
...
#  for (item in unique(df$impact)) {
    subdf <- df[df$region == nuts[rr] & df$impact == "Agriculture_(EPIC)",]
    if (nrow(subdf)==0){
      print(paste("Skipping", nuts[rr], "Agriculture_(EPIC)"))
      next 
    }
      print(paste("Processing", nuts[rr], "Agriculture_(EPIC)"))
...
gp <- ggplot(ddf, aes(T, -mu)) +
  ...
  theme_bw() + ylab("Damages to agricultural sector (£ million)")
ggsave("../channels/Agriculture/agri-coacch-df.pdf", width=9.5, height=9.5)

save(allla, file="../channels/Agriculture/agriculture-la.RData")
