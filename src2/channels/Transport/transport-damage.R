...
df <- read.csv("~/Open Modeling Group Dropbox/COACCH/COACCH_ICES-CMCC_data/impact-ses.csv")

ncr <-nc_open("cmip5/EC-Earth/rcp85/ts_Amon_EC-EARTH_rcp85_r7i1p1_200601-210012-aggregated.nc")

trans.gcm.rcp <- read.csv("~/Open Modeling Group Dropbox/COACCH/climate/cmip5/Combine-GCM/trans-gcm-rcp.csv") 
trans.gcm.hist <-read.csv("~/Open Modeling Group Dropbox/COACCH/climate/cmip5/Combine-GCM/trans-gcm-hist.csv") 

timer <- trans.gcm.rcp$yyyymm
tsavg.r <-trans.gcm.rcp$tsavg
region.r <- trans.gcm.rcp$NUTS_ID
date.r <- as.Date(paste0(timer, "-15"))

timeh <- trans.gcm.hist$yyyymm
tsavg.h <- trans.gcm.hist$tsavg
region.h <- trans.gcm.hist$NUTS_ID
...
#  for (item in unique(df$impact)) {
    subdf <- df[df$region == nuts[rr] & df$impact == "Transport",]
    if (nrow(subdf)==0){
      print(paste("Skipping", nuts[rr], "Transport"))
      next 
    }
      print(paste("Processing", nuts[rr], "Transport"))
...
gp <- ggplot(ddf, aes(T, -mu)) +
  ...
  geom_point(data=pdf) +
  scale_x_continuous("Difference in temperature from 1860 (C)", expand=c(0, 0)) +
  theme_bw() + ylab("Damages to transport sector (£ million)")
ggsave("../channels/Transport/trans-coacch-df.pdf", width=9.5, height=9.5)

save(allla, file="../channels/Transport/transport-la.RData")
