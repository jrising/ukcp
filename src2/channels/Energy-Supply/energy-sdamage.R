...
for (rr in ukrrs){
  ...
  subdf <- df[df$region == nuts[rr] & df$impact == "Energy_Supply",]
  if (nrow(subdf)==0){
    print(paste("Skipping", nuts[rr], "Energy_Supply"))
    next 
  }
  print(paste("Processing", nuts[rr], "Energy_Supply"))
...
gp <- ggplot(ddf, aes(T, -mu)) +
  ...
  theme_bw() + ylab("Damages to energy supply (£ million)")
ggsave("../channels/Energy-Supply/energy-supply-df.pdf", width=9.5, height=9.5)

save(allla, file="../channels/Energy-Supply/energy-supply-la.RData")
