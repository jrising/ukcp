...
ncr <- nc_open("/Users/ritikakhurana/Open Modeling Group Dropbox/COACCH/climate/cmip5/HadGEM2-ES/ts-HadGEM2-rcp-aggregated.nc")

nch <- nc_open("/Users/ritikakhurana/Open Modeling Group Dropbox/COACCH/climate/cmip5/HadGEM2-ES/ts-HadGEM2-hist-aggregated.nc")
...
#  for (item in unique(df$impact)) {
    subdf <- df[df$region == nuts[rr] & df$impact == "Forestry",]
    if (nrow(subdf)==0){
      print(paste("Skipping", nuts[rr], "Forestry"))
      next 
    }
      print(paste("Processing", nuts[rr], "Forestry"))
...
gp <- ggplot(ddf, aes(T, -mu)) +
  ...
  theme_bw() + ylab("Damages to forestry (£ million)")
ggsave("../channels/Forestry/forestry-coacch-df.pdf", width=9.5, height=9.5)

save(allla, file="../channels/Forestry/Forestry-la.RData")
