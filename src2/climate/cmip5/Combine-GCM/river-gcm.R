...
#Open all historical files (riverine floods)
files= list.files('.',pattern=".+hist.+aggregated.csv", recursive = TRUE, full.names = T) %>%
  stringr::str_subset(., "tas-EC-Earth", negate = TRUE)%>%
  stringr::str_subset(., "CNRM", negate = TRUE)%>%
  stringr::str_subset(., "MPI", negate = TRUE)%>%
  stringr::str_subset(., "MR", negate = TRUE)
...
write.csv(gcm, "Combine-GCM/river-gcm-hist.csv", row.names = FALSE)
...
gcm$tsavg <- rowMeans(gcm[, -1:-2])
write.csv(gcm, "Combine-GCM/river-gcm-rcp.csv", row.names = FALSE)
