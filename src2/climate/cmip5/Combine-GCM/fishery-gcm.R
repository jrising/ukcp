...
stringr::str_subset(., "MIROC-ESM-CHEM", negate = TRUE)%>%
...
write.csv(gcm, "Combine-GCM/fish-gcm-hist.csv", row.names = FALSE)
...
gcm$tsavg <- rowMeans(gcm[, -1:-2])
write.csv(gcm, "Combine-GCM/fish-gcm-rcp.csv", row.names = FALSE)
