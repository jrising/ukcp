...
lst <- list("EC-Earth", "MPI-ESM-LR", "HadGEM2-ES")
...
gcm = NULL
for (i in 1:length(files.keep)) {
  df <-read.csv(files.keep[i])
...
write.csv(gcm, "Combine-GCM/trans-gcm-hist.csv", row.names = FALSE)

#Open all rcp files (agriculture model)

lstrcp <- list("EC-Earth", "MPI-ESM-LR", "HadGEM2-ES")
patrcp <- paste0("\\b(", paste(lstrcp, collapse="|"), ")\\b")
filesrcp= list.files('.',pattern=".+rcp.+aggregated.csv$", recursive = TRUE, full.names = TRUE)
filesrcp.keep <- grep(patrcp, filesrcp, value=TRUE)


gcm = NULL
for (i in 1:length(filesrcp.keep)) {
  df <-read.csv(filesrcp.keep[i])
  df$yyyymm <- sapply(df$time, function(time) substring(time, 1, 7)[[1]])
  names(df)[names(df) == 'tas'] = 'ts'
  df <- df[, c('yyyymm', 'NUTS_ID','ts')]
  if (is.null(gcm)) {
...
gcm$tsavg <- rowMeans(gcm[, -1:-2])
write.csv(gcm, "Combine-GCM/trans-gcm-rcp.csv", row.names = FALSE)
